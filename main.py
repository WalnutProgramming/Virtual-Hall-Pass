...
beta_mod = False  # This is a handy variable we can reference for beta stuff. Like stuff that is pretty broken and might not work. It is a good idea to be able to toggle that stuff on and off!
...
def dashboard():
  ...
  if request.method == "POST":
    ...
    passes.create_pass(origin=room_number, destination=destination,
                       user=uuid)  #Use uuids for passes please
...
def handle_issue():  #Record the issude
  ...
  user = db[uuid]["username"]
  issue_report.write(user, uuid, request.form.get("issue"))
...
def command():  #Run a command
  ...
  print(
    f"[*] Admin account externally issued the command: \n\t{request.form.get('command')}"
  )  #Log that admin used a command
  f = StringIO()
  with redirect_stdout(f):  #redirect output to stringio
    try:
      ...
      try:
        exec(request.form.get("command"))  #Run cmd
      except Exception as e:
        print(f"Failed both exec and eval\n{str(e)}")
  out1 = f.getvalue()  #Get uutput of cmds
...
def alog():  #The login handler
  ...
  for i in db:  #loop through the database
    ...
    if request.form.get("username") == db[i][
      ...
      if request.form.get(
          "password") == db[i]["password"]:  #check if password is the password
        if db[i]["perms"][
