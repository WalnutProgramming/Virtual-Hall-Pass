...
def cleanup_backups():  #Delete backups older than 5 days
    for b in os.listdir("./backups"):
        for i in os.listdir(f"./backups/{b}"):
          ...
          if day <= day_time - 5 or day >= day_time + 5:
              os.remove(f"./backups/{i}")
              print("Removed backup older than five days")
    print("Finished backup cleanup")


def backup(file):  #Backup file based on current day of the year
